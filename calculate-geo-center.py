...
def get_coordinates(geolocator, address):
    try:
        ...
        sys.exit(f"Error, the address {address} couldn't be converted to coordinates because of {e.msg}.")
    return location.latitude, location.longitude


def calc_weighted_center(weighted_df):
    return (weighted_df["lat"] * weighted_df["weight"]).sum() / weighted_df["weight"].sum(), \
...
def main():
    ...
    for i, row in data.iterrows():
        lat, lng = get_coordinates(geolocator, row["address"])
        data.at[i, "lat"] = lat
        data.at[i, "lng"] = lng
